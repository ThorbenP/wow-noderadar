...
A small draggable control window with a start/stop button and a settings button
is shown by default; it can be turned off in the options.

## Options

Esc → Interface → AddOns → NodeRadar, or `/nr options`.
...
mouse interaction switched off for the duration, because their tooltips would
otherwise be read as confirmed nodes. Everything is restored when the pass ends.
...
- **The minimap disappears briefly** during each pass. With a handful of
  candidates that is a few frames every few seconds.

## Reporting a bug

Run `/nr debug`, then `/reload`, and attach
`WTF/Account/<account>/SavedVariables/NodeRadar.lua` to the issue. It records the
scan geometry, the candidate list with distances and confirmation state, and a
rolling log of the last 25 tooltips with the reason each one was accepted or
rejected.
